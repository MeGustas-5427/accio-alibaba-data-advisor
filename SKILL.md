...
description: 通过 Accio 已授权账号的远程 MCP 缓存、检索并调用完整工具目录，也可直接查询 Alibaba.com Data Advisor 店铺经营汇总和询盘访客明细。用于查找 Accio 可用工具及用途、刷新可提交的公开工具目录、按名称或说明筛选工具、调用用户指定工具、验证本地目录与凭据，或在 Accio Token 更新后同步技能凭据；不依赖 localhost:4097，Accio 关闭后仍可在 Token 有效期内使用。
---
...
- 最新 `tools/list` 目录保存在 `references/tools_catalog.json`，可提交到 Git。目录保留工具名称、说明和参数结构，仅清理真实的敏感默认值、示例值或凭据值。
...
4. 运行 `self-test`，要求凭据可用、完整目录可读且数量一致；该动作不重复请求远程 `tools/list`。
5. 用 `list-tools` 从本地目录列出或检索工具；每项保留用途说明和输入结构。
6. 根据请求运行便捷动作 `shop-summary` / `visitor-detail`，或用 `call-tool` 调用本地目录中的指定工具。
7. 最终只汇报工具、查询范围、分页、返回记录或指标；不要汇报凭据内容。
